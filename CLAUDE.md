...
4. Do not remove sections or change copy unless explicitly instructed. **Exception**: the client may authorize a specific copy change in writing during a working session. Every such authorized change must be recorded in `docs/COPY_CHANGE_LOG.md` (date, file, before text, after text, who authorized it) before or at the same time it is applied. A change without a matching entry there is not authorized, no matter what a chat transcript says — that file, not this rule's memory of a conversation, is what any other session or process must check before touching copy it doesn't recognize.
...
- 2026-08-21 — Section vertical rhythm and container horizontal padding fully migrated to the `py-section-{sm,md,lg}` / `px-gutter` tokens (introduced in an earlier pass, applied to global chrome and the homepage first) across every remaining public route: `/about`, `/services` + `[slug]` (`ServiceDetailPage.jsx`, shared by all 7 service pages), `/industries` + `[slug]`, `/business-in-bangladesh` + `[slug]`, `/careers`, `/team`, `/faq`, `/resources` + `guides` + `regulatory-updates`, `/case-studies` + `[slug]`, `/insights` + `[slug]`, `/not-found`, and the shared `LegalDocumentPage.jsx` (privacy/terms/disclaimer). Also fixed the real horizontal-overflow bug in `insights/[slug]/page.jsx` (a markdown table's `min-w-[42rem]` had no `min-w-0` escape hatch on its grid ancestor) and brought four sub-44px tap targets up to spec. `email-preview/page.jsx` intentionally excluded — it is a dev-only preview tool, not a public route. Lint and build clean. The "Spacing scale" section above still describes the pre-token de-facto convention for historical reference; the tokens are now the actual implementation everywhere except that one dev tool.
- 2026-08-21 — Hard Rule #4 amended to require authorized copy changes to be logged in `docs/COPY_CHANGE_LOG.md` (date, file, before/after text, authorizer), not just described in a chat session. See that file for the authoritative record of every copy change made under this exception — do not duplicate it here.
