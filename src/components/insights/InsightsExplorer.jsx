    ...
    <>
      <div className="flex flex-col gap-5 border-y border-border py-6 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-wrap gap-2">
          {categories.map((item) => (
            <button key={item} type="button" onClick={() => setCategory(item)} className={`inline-flex min-h-11 items-center rounded-full border px-3.5 text-[0.68rem] font-extrabold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-blue/40 ${category === item ? "border-brand-navy bg-brand-navy text-white" : "border-border bg-surface text-muted hover:border-brand-blue/35 hover:text-brand-blue"}`}>{item}</button>
          ))}
        </div>
      ...
      <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {filtered.map((item, index) => (
          <article key={item.slug} className={`group overflow-hidden rounded-[1.4rem] border border-border bg-surface transition duration-300 hover:-translate-y-1 hover:border-brand-blue/30 hover:shadow-[0_22px_60px_rgba(27,20,100,.10)] ${index === 0 && category === "All" && !query ? "md:col-span-2" : ""}`}>
            <Link href={`/insights/${item.slug}`} className="flex h-full min-h-[26rem] flex-col focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-gold/60">
