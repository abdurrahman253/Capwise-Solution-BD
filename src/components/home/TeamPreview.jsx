    ...
    <section className="border-y border-border bg-background py-18 sm:py-24 lg:py-28" aria-labelledby="team-preview-title">
      <div className="mx-auto max-w-[94rem] px-5 sm:px-6 lg:px-8 2xl:px-10">
        <div className="grid gap-8 lg:grid-cols-12 lg:items-end">
          <div className="lg:col-span-8"><p className="text-[0.62rem] font-extrabold uppercase tracking-[0.2em] text-brand-blue">Our Team</p><h2 id="team-preview-title" className="mt-5 max-w-[13ch] font-display text-[clamp(1.75rem,2.6vw,2.5rem)] font-semibold leading-[1.1] tracking-[-0.02em] text-foreground">Two lead profiles. One connected team.</h2></div>
          ...
          {featuredTeamMembers.map((member, index) => (
            <m.article key={member.slug} initial={reduceMotion ? false : { opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .2 }} transition={{ delay: index * .06 }} className="group overflow-hidden rounded-[1.3rem] bg-brand-navy p-5 text-white shadow-[0_26px_75px_rgba(27,20,100,.16)] sm:rounded-[1.55rem] sm:p-7 lg:p-9">
              <Link href={`/team#${member.slug}`} className="flex min-h-[13rem] flex-col focus-visible:outline-none sm:min-h-[18rem] lg:min-h-[21rem]">
                ...
                <div className="mt-auto pt-6 sm:pt-10"><p className="text-[0.6rem] font-extrabold uppercase tracking-[0.14em] text-brand-gold sm:text-[0.62rem] sm:tracking-[0.16em]">{member.role}</p><h3 className="mt-2 font-display text-[clamp(1.2rem,1.5vw,1.55rem)] font-semibold leading-[1.1] tracking-[-0.02em] sm:mt-3">{member.name}</h3><p className="mt-2 max-w-xl text-xs leading-6 text-white/65 sm:mt-3 sm:text-sm sm:leading-7">{member.specialty}</p><div className="mt-4 flex items-center justify-between border-t border-white/12 pt-4 text-xs font-bold text-white/64 sm:mt-6 sm:pt-5"><span>{member.experience}</span><ArrowUpRight size={16} className="text-brand-gold transition group-hover:-translate-y-0.5 group-hover:translate-x-0.5" /></div></div>
              </Link>
            </m.article>
          ))}
        </div>
