...
const markdownComponents = {
  h2: ({ children }) => <h2 className="mt-11 scroll-mt-28 font-display text-[clamp(1.75rem,2.6vw,2.5rem)] font-semibold leading-[1.1] tracking-[-0.02em] text-foreground">{children}</h2>,
  h3: ({ children }) => <h3 className="mt-8 scroll-mt-28 font-display text-[clamp(1.35rem,1.8vw,1.8rem)] font-semibold leading-[1.1] tracking-[-0.02em] text-foreground">{children}</h3>,
      ...
      <main id="main-content" className="bg-background">
        <article>
          <header className="border-b border-border bg-surface py-12 sm:py-16 lg:py-20">
            <div className="mx-auto max-w-[94rem] px-5 sm:px-6 lg:px-8 2xl:px-10">
              ...
              <div className="mt-8 grid gap-10 lg:grid-cols-12 lg:items-end">
                <div className="lg:col-span-7"><p className="text-[0.62rem] font-extrabold uppercase tracking-[0.19em] text-brand-blue">{insight.category}</p><h1 className="mt-4 max-w-[16ch] font-display text-[clamp(2rem,3.5vw,3.2rem)] font-semibold leading-[1.1] tracking-[-0.02em] text-foreground">{insight.heading || insight.title}</h1><p className="mt-6 max-w-3xl text-base leading-8 text-muted sm:text-lg sm:leading-9">{insight.description}</p></div>
          ...
          <div className="mx-auto grid max-w-[94rem] gap-12 px-5 py-12 sm:px-6 sm:py-16 lg:grid-cols-12 lg:px-8 lg:py-20 2xl:px-10">
            ...
            <div className="lg:col-span-7"><div className="capwise-article max-w-[48rem]"><ReactMarkdown remarkPlugins={[remarkGfm]} components={markdownComponents}>{content}</ReactMarkdown></div>
              <div className="mt-14 rounded-[1.5rem] bg-brand-navy p-7 text-white sm:p-9"><p className="text-[0.6rem] font-extrabold uppercase tracking-[.18em] text-brand-gold">One practical next step</p><h2 className="mt-4 font-display text-[clamp(1.35rem,1.8vw,1.8rem)] font-semibold leading-[1.1] tracking-[-0.02em]">Need help applying this to your business?</h2><p className="mt-4 max-w-2xl text-sm leading-7 text-white/65">Capwise can review your current position and help identify the registrations, filings or decisions that need attention first.</p><Link href="/contact" className="mt-7 inline-flex min-h-12 items-center gap-3 rounded-full bg-brand-gold px-6 text-sm font-extrabold text-brand-navy transition hover:-translate-y-0.5 hover:bg-brand-gold-soft">Book a Free Consultation <ArrowRight size={16}/></Link></div>
            </div>
          </div>
        </article>

        {related.length > 0 && <section className="border-t border-border bg-surface-muted/55 py-14 sm:py-18"><div className="mx-auto max-w-[94rem] px-5 sm:px-6 lg:px-8 2xl:px-10"><div className="flex items-end justify-between gap-6"><div><p className="text-[0.6rem] font-extrabold uppercase tracking-[.18em] text-brand-blue">Continue reading</p><h2 className="mt-3 font-display text-[clamp(1.75rem,2.6vw,2.5rem)] font-semibold leading-[1.1] tracking-[-0.02em] text-foreground">Related insights</h2></div><Link href="/insights" className="hidden text-sm font-extrabold text-brand-blue sm:inline-flex">All insights</Link></div><div className="mt-8 grid gap-4 md:grid-cols-3">{related.map((item)=><Link key={item.slug} href={`/insights/${item.slug}`} className="group rounded-[1.2rem] border border-border bg-surface p-6 transition hover:-translate-y-1 hover:border-brand-blue/30"><p className="text-[0.58rem] font-extrabold uppercase tracking-[.15em] text-brand-blue">{item.category}</p><h3 className="mt-3 font-display text-[clamp(1.35rem,1.8vw,1.8rem)] font-semibold leading-[1.1] tracking-[-0.02em] text-foreground group-hover:text-brand-blue">{item.title}</h3><span className="mt-6 inline-flex items-center gap-2 text-xs font-extrabold text-foreground">Read <ArrowRight size={14} className="text-brand-gold"/></span></Link>)}</div></div></section>}
      </main>
    </>
  );
}
