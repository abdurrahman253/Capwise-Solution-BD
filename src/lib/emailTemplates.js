...
<body style="margin:0;padding:0;background:#F3F5FA;font-family:Arial,Helvetica,sans-serif;color:${brand.text};">
  <div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent;">${escapeHtml(preheader)}</div>
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#F3F5FA;padding:28px 12px;">
    <tr><td align="center">
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:680px;background:#FFFFFF;border:1px solid ${brand.border};border-radius:20px;overflow:hidden;box-shadow:0 24px 70px rgba(27,20,100,.12);">
        <tr>
            ...
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td>
                  ...
                  <div style="margin-top:2px;font-size:9px;line-height:13px;font-weight:700;letter-spacing:.22em;color:${brand.accent};">SOLUTION BD</div>
                </td>
        ...
        <tr>
          <td style="padding:36px 34px 16px;">
            <div style="font-size:10px;line-height:16px;font-weight:800;letter-spacing:.18em;text-transform:uppercase;color:${brand.blue};">${escapeHtml(eyebrow)}</div>
            ...
            ${reference ? `<div style="display:inline-block;margin-top:18px;padding:8px 12px;border-radius:999px;background:${brand.pale};font-size:11px;line-height:16px;font-weight:800;letter-spacing:.08em;color:${brand.blue};">REFERENCE&nbsp; ${escapeHtml(reference)}</div>` : ""}
          </td>
        </tr>
...
export function consultationOwnerEmail({ reference, data, submittedAt }) {
  ...
  const content = `
    ...
    <div style="margin-top:24px;padding:22px;border:1px solid ${brand.border};border-radius:14px;background:#F8FAFC;">
      <div style="font-size:10px;line-height:16px;font-weight:800;letter-spacing:.16em;text-transform:uppercase;color:${brand.blue};">Client briefing</div>
      <div style="margin-top:10px;font-size:14px;line-height:24px;color:${brand.text};">${message}</div>
    </div>
    <table role="presentation" cellpadding="0" cellspacing="0" style="margin-top:22px;">
      <tr>
        <td style="padding-right:10px;">
          <a href="${replyHref}" style="display:inline-block;padding:12px 18px;border-radius:999px;background:${brand.blue};font-size:12px;line-height:18px;font-weight:800;text-decoration:none;color:#FFFFFF;">Reply to visitor</a>
        </td>
...
export function resourceOwnerEmail({ reference, data, submittedAt }) {
  const content = `
    ...
    <div style="margin-top:22px;padding:16px 18px;border-left:3px solid ${brand.accent};background:${brand.pale};font-size:12px;line-height:20px;color:${brand.muted};">This is an interest request only. Do not promise a release date until the guide has completed content, legal and client approval.</div>`;
...
export function newsletterOwnerEmail({ reference, data, submittedAt }) {
  const content = `
    ...
    <div style="margin-top:22px;padding:16px 18px;border-left:3px solid ${brand.accent};background:${brand.pale};font-size:12px;line-height:20px;color:${brand.muted};">Only send reviewed regulatory updates with an official source, a visible last-reviewed date and an unsubscribe process.</div>`;
